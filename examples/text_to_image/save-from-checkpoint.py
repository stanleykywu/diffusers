...
def log_validation(
    ...
    if args.eval_fid and global_step % args.fid_validation_steps == 0:
        if global_step == 0 and os.path.exists(args.untrain_fid_dir):
            validation_dir = args.untrain_fid_dir
        else:
            validation_dir = os.path.join(
                args.output_dir, f"fid-validation-{global_step}steps"
            )
            os.makedirs(validation_dir, exist_ok=True)
            for i in tqdm(
                range(len(args.fid_prompts)), desc="Generating FID validation"
            ):
                output_fp = os.path.join(
                    validation_dir, f"{args.fid_prompts[i]['image_id']}.png"
                )
        ...
        for tracker in accelerator.trackers:
            if tracker.name == "wandb":
                ...
                logger.warning(
                    f"FID/Aesthetic/CLIP logging not implemented for {tracker.name}"
                )
...
def main():
    ...
    args.validation_prompts = poison_config["validation_prompts"]
    ...
    validate = True if (args.validation_prompts or args.fid_prompts) else False
    ...
    for poison_concept_dta in poison_config["poison"]:
        poison_name = poison_concept_dta["poison_data_dir"].split("/")[-1]
        poison_data_files = {
            "train": os.path.join(poison_concept_dta["poison_data_dir"], "**")
        }
        poison_dataset = load_dataset(
            "imagefolder",
            data_files=poison_data_files,
            cache_dir=args.cache_dir,
        )["train"].shuffle(seed=args.seed)

        poison_amount = int(total_training_amount * poison_concept_dta["percentage"])
        assert poison_amount > 0
        logger.info(f"{poison_amount} of poison data for {poison_name}")
        total_poison_amount += poison_amount
        poison_subset = poison_dataset.select(range(poison_amount))
        poison_datasets.append(poison_subset)

    clean_amount = total_training_amount - total_poison_amount
    assert clean_amount > 0
    logger.info(f"{clean_amount} of clean data")
    clean_subset = clean_dataset.select(range(clean_amount))

    combined_dataset = concatenate_datasets(poison_datasets + [clean_subset]).shuffle(
        seed=args.seed
    )
    ...
    if accelerator.is_main_process:
        ...
        pipeline.save_pretrained(
            "/bigstor/stanleywu/diffusion-ft/models/sd2.1/[HYPERPARAMETER-BENCHMARK]-sd21-text-to-image/25000total-clean-warmup1-188459528seed-15000steps-0.0001lr-16bs-4gradaccum"
        )
